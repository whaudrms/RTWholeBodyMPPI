...
def arm_actuator_indices(agent: MPPI) -> np.ndarray:
    ...
    expected = len(agent.arm_kinematics.arm_joint_ids)
    if len(indices) != expected:
        raise ValueError(f"Expected {expected} arm actuators, got {indices}")
    return indices
...
def push_cost_without_arm_objective(
    ...
    flat_robot_states = agent.canonical_robot_state(
        flat_robot_states,
        robot_qpos_indices=agent.robot_qpos_indices,
        robot_dof_indices=agent.robot_dof_indices,
    )
    ...
    base_velocity_start = 7 + agent.act_dim
    flat_robot_states[:, base_velocity_start:base_velocity_start + 3] = (
        batch_world_to_local_velocity(
            flat_robot_states[:, 3:7],
            flat_robot_states[:, base_velocity_start:base_velocity_start + 3],
        )
    )
...
def validate_fairness(logs: list[EpisodeLog]) -> None:
    ...
    for seed, by_mode in groups.items():
        ...
        arm_dof = len(nominal.arm_qpos_indices)
        arm_indices = np.arange(len(nominal.base_noise_sigma) - arm_dof,
                                len(nominal.base_noise_sigma))
        leg_indices = np.arange(len(nominal.base_noise_sigma) - arm_dof)
        np.testing.assert_array_equal(
