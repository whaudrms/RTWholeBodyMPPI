...
def arm_actuator_indices(agent: MPPI) -> np.ndarray:
    """Resolve the arm actuator indices from the MuJoCo model."""
    ...
    if len(indices) != expected:
        raise ValueError(
            f"Expected {expected} arm actuators, found "
...
def leg_only_cost(
    ...
    flat_states = agent.canonical_robot_state(flat_states)
...
def disable_arm_objective(
    ...
    # Compact state layout: base pose error 6, actuator q,
    # base velocity 6, actuator dq.
    arm_q_weights = 6 + arm_indices
    arm_dq_weights = 12 + agent.act_dim + arm_indices
