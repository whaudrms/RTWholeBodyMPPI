"""Convert legacy Go1 gait TSVs into Go2 + SO-ARM100 references.

Two conversion modes are available:

``padding``
    Preserve all 12 Go1 leg position/velocity rows exactly and append four
    fixed SO-ARM100 positions plus four zero arm velocities. This is the legacy
    behavior.
...
DEFAULT_ARM_POSITION = np.array(
    [0.0, -1.57, 1.57, 1.57], dtype=float
)
...
def default_output(source: Path, mode: str = "padding") -> Path:
    ...
    if mode == "padding":
        return source.parent.parent / "go2_so_arm" / source.name
    return source.parent.parent / "go2_so_arm_retargeted" / source.name
...
def convert(
    ...
    if mode == "padding":
        ...
        header = (
            "Go2 + SO-ARM100 dimension-padded gait; source leg rows preserved; "
        ...
        header = (
            "Go2 + SO-ARM100 kinematically transferred gait; foot x/y "
            "centered on the Go2 keyframe, stance grounded, "
            f"base height={height_label}, and swing "
            f"height={target_height:.6g}m; source Go2-FK height={source_height:.6g}m; "
