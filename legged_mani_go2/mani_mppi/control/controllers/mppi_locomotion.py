...
GAIT_INPLACE_PATH = os.path.join(
    GAIT_DIR, "FAST/go2_so_arm/walking_gait_raibert_FAST_0_0_15cm_100hz.tsv"
)
GAIT_TROT_PATH = os.path.join(
    GAIT_DIR, "FAST/go2_so_arm/walking_gait_raibert_FAST_0_5_15cm_100hz.tsv"
)
GAIT_WALK_PATH = os.path.join(
    GAIT_DIR, "FAST/go2_so_arm/walking_gait_raibert_FAST_0_1_15cm_100hz.tsv"
)
...
class MPPI(GaitNominalMPPI):
    ...
    def quadruped_cost_np(self, x, u, x_ref):
        ...
        joint_stop = 7 + self.act_dim
        joint_velocity_start = joint_stop + 6
        x_joint = x[:, 7:joint_stop]
        v_joint = x[:, joint_velocity_start:joint_velocity_start + self.act_dim]
        uv = x_ref[:, joint_velocity_start:joint_velocity_start + self.act_dim]
    ...
    def calculate_total_cost(self, states, actions, joints_ref, body_ref):
        ...
        states = self.canonical_robot_state(states)

        # Repeat and reshape the model-sized [q, dq] joint reference.
        ...
        # Canonical state order: base qpos, actuator q, base dq, actuator dq.
        x_ref = np.concatenate(
            [
                traj_body_ref[:, :7],
                joints_ref[:, :self.act_dim],
                base_velocity_ref,
                joints_ref[:, self.act_dim:],
            ],
            axis=1,
        ...
        base_velocity_start = 7 + self.act_dim
        rotated_ref = batch_world_to_local_velocity(
            states[:, 3:7],
            states[:, base_velocity_start:base_velocity_start + 3],
        )
        states[:, base_velocity_start:base_velocity_start + 3] = rotated_ref
