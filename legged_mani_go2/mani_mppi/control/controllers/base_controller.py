...
class BaseMPPI:
    ...
    Go2 + SO-ARM100 model so task controllers define only cost and update.
    """
    ...
    def __init__(self, model_path: str | Path, config_path: str | Path):
        ...
        if np.any(actuator_joint_ids < 0):
            raise ValueError("Every MPPI actuator must target one joint")
        self.joint_qpos_indices = self.model.jnt_qposadr[
            actuator_joint_ids
        ].copy()
        self.joint_dof_indices = self.model.jnt_dofadr[
            actuator_joint_ids
        ].copy()
        ...
        if base_body_id < 0:
            raise ValueError("Model is missing the base_link body")
        base_joint_id = int(self.model.body_jntadr[base_body_id])
        if (
            base_joint_id < 0
            or self.model.jnt_type[base_joint_id]
            != mujoco.mjtJoint.mjJNT_FREE
        ):
            raise ValueError("base_link must have a free joint")
        self.base_qpos_indices = np.arange(
            self.model.jnt_qposadr[base_joint_id],
            self.model.jnt_qposadr[base_joint_id] + 7,
        )
        self.base_dof_indices = np.arange(
            self.model.jnt_dofadr[base_joint_id],
            self.model.jnt_dofadr[base_joint_id] + 6,
        )
    ...
    @staticmethod
    def _local_indices(global_indices, selected_indices):
        """Map model-global qpos/dof addresses into an extracted state."""
        lookup = {
        ...
        try:
            return np.asarray(
                [lookup[int(index)] for index in global_indices], dtype=int
            )
        except KeyError as error:
            raise ValueError(
                f"Extracted robot state is missing model index {error.args[0]}"
            ) from error

    def canonical_robot_state(
        self,
        states: np.ndarray,
        *,
        robot_qpos_indices: np.ndarray | None = None,
        robot_dof_indices: np.ndarray | None = None,
    ) -> np.ndarray:
        """Return base and actuated joints in MPPI actuator order.

        MuJoCo stores joints in body-tree order, while the Go2 actuator and gait
        order is FR, FL, RR, RL, followed by the six arm joints.  Cost code
        uses this compact canonical layout independently of MJCF body order.
        """
        states = np.asarray(states, dtype=float)
        if states.ndim != 2:
            raise ValueError("states must be a two-dimensional array")

        if robot_qpos_indices is None:
            qpos_count = self.model.nq
            dof_count = self.model.nv
            if states.shape[1] != qpos_count + dof_count:
                raise ValueError(
                    "Full robot state has the wrong width: "
                    f"{states.shape[1]} != {qpos_count + dof_count}"
                )
            base_qpos = self.base_qpos_indices
            joint_qpos = self.joint_qpos_indices
            base_dof = self.base_dof_indices
            joint_dof = self.joint_dof_indices
        else:
            if robot_dof_indices is None:
                raise ValueError(
                    "robot_dof_indices is required with robot_qpos_indices"
                )
            robot_qpos_indices = np.asarray(robot_qpos_indices, dtype=int)
            robot_dof_indices = np.asarray(robot_dof_indices, dtype=int)
            qpos_count = len(robot_qpos_indices)
            dof_count = len(robot_dof_indices)
            if states.shape[1] != qpos_count + dof_count:
                raise ValueError(
                    "Extracted robot state has the wrong width: "
                    f"{states.shape[1]} != {qpos_count + dof_count}"
                )
            base_qpos = self._local_indices(
                self.base_qpos_indices, robot_qpos_indices
            )
            joint_qpos = self._local_indices(
                self.joint_qpos_indices, robot_qpos_indices
            )
            base_dof = self._local_indices(
                self.base_dof_indices, robot_dof_indices
            )
            joint_dof = self._local_indices(
                self.joint_dof_indices, robot_dof_indices
            )

        qpos = states[:, :qpos_count]
        qvel = states[:, qpos_count:]
        ...
        expected_width = 13 + 2 * self.act_dim
        if canonical.shape[1] != expected_width:
            raise ValueError(
                f"Canonical state width must be {expected_width}, got "
                f"{canonical.shape[1]}"
            )
        return canonical
