"""Shared gait, arm-kinematics, and collision support for Go2 + SO-ARM100."""
...
GAIT_PATHS = {
    ...
    "trot": os.path.join(
        GAIT_DIR,
        "MED/go2_so_arm/walking_gait_raibert_MED_0_5_15cm_100hz.tsv",
    ),
    "walk": os.path.join(
        GAIT_DIR,
        "MED/go2_so_arm/walking_gait_raibert_MED_0_1_10cm_100hz.tsv",
    ),
    "walk_fast": os.path.join(
        GAIT_DIR,
        "FAST/go2_so_arm/walking_gait_raibert_FAST_0_1_10cm_100hz.tsv",
    ),
    "stance_hold": os.path.join(
        GAIT_DIR,
        "FAST/go2_so_arm_height_conditioned/h_0p270/stance_hold.tsv",
    ),
}
HEIGHT_GAIT_DIR = os.path.join(GAIT_DIR, "FAST/go2_so_arm_height_conditioned")
HEIGHT_GAIT_LEVELS = {
    0.27: "h_0p270",
    0.225: "h_0p225",
    0.18: "h_0p180",
...
class WholeBodyArmMPPI(GaitNominalMPPI):
    ...
    def _compact_robot_state_error(self, states, references):
        """Return the compact robot error without a dummy quaternion slot."""
    ...
    def _joint_reference(self):
        ...
        reference = np.vstack(
            (
                gait[:12],
                arm_q,
                gait[self.act_dim:self.act_dim + 12],
                arm_dq,
            )
        )
