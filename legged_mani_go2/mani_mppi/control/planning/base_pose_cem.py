...
class KinematicBasePoseCEMPlanner:
    ...
    def __init__(
        ...
        *,
        ...
        self.stand_base_height = float(
            config.get("stand_base_height", 0.27)
        )
    ...
    def _evaluate_candidates(
        ...
        arm_point_count = len(self.arm_kinematics.arm_fk_body_ids) + 1
        arm_dof = len(self.arm_kinematics.arm_qpos_indices)
        arm_positions = np.empty(
            (num_candidates, arm_point_count, 3), dtype=float
        )
        arm_q_samples = np.empty((num_candidates, arm_dof), dtype=float)
        ...
        for index, candidate in enumerate(candidates):
            ...
            arm_positions[index, :-1] = data.xpos[
                self.arm_kinematics.arm_fk_body_ids
            ]
            arm_positions[index, -1] = data.site_xpos[
