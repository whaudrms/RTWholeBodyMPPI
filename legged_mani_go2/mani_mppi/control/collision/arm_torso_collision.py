"""Fast capsule-to-torso clearance for the Go2 + SO-ARM100 arm."""
...
class ArmTorsoCollision:
    """Evaluate an arm capsule chain against a torso-oriented box.
    ...
    def __init__(self, model: mujoco.MjModel, config: dict) -> None:
        ...
        self.capsule_radii = np.asarray(
            config.get(
                "collision_capsule_radii",
                [0.035, 0.035, 0.030, 0.025],
            ),
            dtype=float,
        )
        if self.capsule_radii.ndim != 1 or len(self.capsule_radii) < 1:
            raise ValueError(
                "collision_capsule_radii must be a non-empty vector"
            )
    ...
    def evaluate(
        ...
        expected_shape = (
            len(flat_states), len(self.capsule_radii) + 1, 3
        )
        ...
        starts_world = arm_positions[:, :-1].copy()
        ...
        segment_count = len(self.capsule_radii)
        starts_geom = endpoints_geom[:, :segment_count]
        ends_geom = endpoints_geom[:, segment_count:]
