"""B2-Z1 arm IK, FK, and rollout-sensor access."""
...
ARM_BODY_NAMES = ("link02", "link03", "link04")
...
class ArmKinematics:
    """Own the reusable MuJoCo data and indices for the four-DoF arm."""
    ...
    def batch_arm_fk(
        ...
        """Evaluate shoulder, elbow, wrist, and EE frames for each state."""
        ...
        for index, state in enumerate(flat_states):
            ...
            positions[index, :3] = self.fk_data.xpos[self.arm_fk_body_ids]
            positions[index, 3] = self.fk_data.site_xpos[self.ee_site_id]
            mujoco.mju_mat2Quat(
                quaternions[index],
    ...
    def batch_ee_pose(
        ...
        """Evaluate EE poses through the shared four-point FK path."""
