...
class BaseMPPI:
    ...
    def __init__(self, model_path, config_path):
        ...
        if (
            ...
            raise ValueError(
                "The Go2 controller requires 12 actuators and either the "
                "robot-only state (nq=19, nv=18) or one preceding free "
                "object (nq=26, nv=24)."
            )
        ...
        # Initialize the nominal command from the selected Go2 scene instead
        # of carrying over robot-specific crouch constants. Dynamic-object
        # scenes such as push-box expose only ``home``; locomotion scenes also
        # expose ``sit``.
        sampling_key = params.get('sampling_keyframe', 'sit')
        key_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_KEY, sampling_key
        )
        if key_id < 0:
            key_id = mujoco.mj_name2id(
        ...
        if key_id < 0:
            raise ValueError(
                "The Go2 model must define a 'home' keyframe for MPPI "
                "initialization."
            )
        ...
        # stores Go2 joints in body-tree order (FL, FR, RL, RR). Store offsets
        # within the robot joint block so this also works when another free
        # body (the push box) precedes the robot in the full model state.
        self.act_dim = self.model.nu
        actuator_joint_ids = self.model.actuator_trnid[:, 0]
        actuator_qpos_adrs = self.model.jnt_qposadr[actuator_joint_ids]
        actuator_dof_adrs = self.model.jnt_dofadr[actuator_joint_ids]
        self.joint_qpos_offsets = actuator_qpos_adrs - actuator_qpos_adrs.min()
        self.joint_qvel_offsets = actuator_dof_adrs - actuator_dof_adrs.min()
        expected_offsets = np.arange(self.act_dim)
        if (
            not np.array_equal(np.sort(self.joint_qpos_offsets), expected_offsets)
            or not np.array_equal(np.sort(self.joint_qvel_offsets), expected_offsets)
        ):
            raise ValueError(
                "Go2 actuated joints must form contiguous qpos/qvel blocks."
            )

        # Read limits directly from the selected Go2 model.
    ...
    def state_in_actuator_order(self, state):
        """Return an extracted 37-D robot state in controller joint order."""
        state = np.asarray(state)
        if state.ndim != 2 or state.shape[1] != 37:
            raise ValueError(
                f"Expected robot state shape (N, 37), got {state.shape}"
            )
        ordered_state = state.copy()
        ordered_state[:, 7:19] = state[:, 7 + self.joint_qpos_offsets]
        ordered_state[:, 25:37] = state[:, 25 + self.joint_qvel_offsets]
    ...
    def threaded_rollout(self, state, ctrl, initial_state, num_workers=32, nstep=5):
        ...
        """
        num_workers = max(1, min(int(num_workers), len(initial_state)))
