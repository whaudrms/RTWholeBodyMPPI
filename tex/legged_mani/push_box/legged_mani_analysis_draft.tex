...
Gaussian perturbations are drawn at four temporal knots and only the
perturbation is interpolated with a cubic spline:
\begin{align}
  \bm{\epsilon}_{n,j}
  &\sim\mathcal{N}(\bm{0},\diag(\bm{\sigma}^2)),\\
  \tilde{\bm{\epsilon}}_{n,0:H-1}
  &=\operatorname{CubicSpline}(\bm{\epsilon}_{n,1:4}),\\
  \bm{u}_{n,k}
  &=\clip\left(
  ...
  \right).
\end{align}
The base standard deviations are $0.03$ for each hip, $0.10$ for each thigh
and calf, and $0.06\,\mathrm{rad}$ for each arm joint. The
\code{stance\_hold} gait multiplies these values by $0.6$; \code{walk\_fast}
uses scale $1.0$. Candidate zero is replaced with the clipped noise-free
nominal trajectory.
...
\begin{algorithmic}[1]
  ...
  \STATE Combine height-conditioned gait and arm IK with the shifted MPPI
         correction.
  \STATE Generate $N=30$ cubic-noise candidate sequences and preserve one
         noise-free nominal candidate.
  ...
  \ENDIF
  \STATE Shift the gait/IK correction and advance gait phase.
